...
import { IRequestUser } from "../../../interfaces/index.d";
import AppError from "../../helpers/errorHelpers/AppError";
import prisma from "../../lib/prisma";

const buildRatingWiseBarChart = (ratings: number[]) => {
...
const getMonthlyReviewCreations = async () => {
  const data = await prisma.$queryRaw<Array<{ month: Date; count: number }>>`
...
const getConsumerStatsData = async (user: IRequestUser) => {
  const [
    totalReviewsWritten,
    totalLikes,
    totalDishes,
...
const getAdminStatsData = async () => {
  ...
  const roleCountMap = groupedUsers.reduce<Record<string, number>>((acc, item) => {
  ...
  return {
    ...
    totalReviews,
    totalRestaurants,
    ratingWiseRestaurantBarChart: buildRatingWiseBarChart(
      restaurantRatings.map((item) => item.ratingAvg),
    ),
    ratingWiseDishBarChart: buildRatingWiseBarChart(
      dishRatings.map((item) => item.ratingAvg),
    ),
    monthlyReviewCreations,
    monthlyUserRegistration,
  };
...
const getOwnerStatsData = async (user: IRequestUser) => {
  ...
  return {
    ...
    ratingWiseDishesBarChart: buildRatingWiseBarChart(
      dishRatings.map((item) => item.ratingAvg),
    ),
  };
};
