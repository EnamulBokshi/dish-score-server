...
type SearchScope = "all" | "restaurants" | "dishes" | "reviews";
...
const toSortOrder = (value: unknown): Prisma.SortOrder => {
...
const paginateArray = <T>(data: T[], page: number, limit: number) => {
  const skip = (page - 1) * limit;
  return data.slice(skip, skip + limit);
};

const shouldInclude = (scope: SearchScope, target: Exclude<SearchScope, "all">) => {
  return scope === "all" || scope === target;
};

const matchesRestaurantSearchTerm = (
  item: {
    name: string;
...
const matchesDishSearchTerm = (
  item: {
    ...
    tags: string[];
    ingredients: string[];
    reviewComments: Array<string | null>;
  },
  term: string,
) => {
  return (
...
const matchesReviewSearchTerm = (
  ...
  return (
    includesTerm(item.comment, term) ||
...
const getGlobalSearchResults = async (query: IQueryParams) => {
  ...
  const scope: SearchScope = ["all", "restaurants", "dishes", "reviews"].includes(scopeRaw)
  ...
  if (normalizedSearchTerm) {
    ...
    const filteredRestaurants = restaurantsRaw.filter((item) =>
      matchesRestaurantSearchTerm(
        {
          name: item.name,
    ...
    const filteredDishes = dishesRaw.filter((item) =>
    ...
    const filteredReviews = reviewsRaw.filter((item) =>
      matchesReviewSearchTerm(
        {
  ...
  const [[restaurantTotal, restaurants], [dishTotal, dishes], [reviewTotal, reviews]] =
    await Promise.all([restaurantsPromise, dishesPromise, reviewsPromise]);
